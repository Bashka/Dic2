...
Если вы впервые пользуетесь данным менеджером, он выполнит первоначальное конфигурирование системы. Сюда входит выбор каталога, используемого для хранения устанавливаемых модулей. В качестве локального каталога пользователя CPAN используется каталог `~/.cpan`. В этом же каталоге хранится файл конфигурации менеджера по адресу `~/.cpan/CPAN/MyConfig.pm`.
...
Для последней операции могут потребоваться права суперпользователя, что позволит сохранить файлы модуля и документацию в общую библиотеку.

Локальная библиотека
--------------------

По умолчанию оба типа установки модуля используют общую библиотеку Perl. Если необходимо использовать локальную, пользовательскую библиотеку необходимо:

1. Указать Perl расположение пользовательской библиотеки можно с помощью переменной окружения `PERL5LIB=каталог`;
1. Использовать переменную окружения при сборке make файла `perl Makefile.PL INSTALL_BASE=каталог`.

Если используется модуль CPAN, можно указать переменную окружения при сборке make файла в файле конфигурации модуля `~/.cpan/CPAN/MyConfig.pm` свойстве `makepl_arg`. Ниже приведен пример значения такого свойства:

    'makepl_arg' => q[INSTALLDIRS=site INSTALL_BASE=/home/username/lib]
