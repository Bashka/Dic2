...
* `set опция` - данная команда задает значение для конкретной опции (команда `setlocal опция` задает значение опции только для текущего буфера). В отличии от `:options`, который распространяется только на данный файл, указание `set` в файлах настроек позволяет закрепить значения за опциями постоянно.
